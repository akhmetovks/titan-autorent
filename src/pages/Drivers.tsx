  ...
  return (
    <div className="p-4 md:p-8">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h2 className="text-2xl font-bold text-white tracking-tight">Водители</h2>
        <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-gray-900 border border-gray-800 rounded-xl shadow-lg shadow-black/20 p-6 mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {drivers.map(driver => (
          <div key={driver.id} className="bg-gray-900 border border-gray-800 rounded-xl shadow-lg shadow-black/20 p-5 flex items-center justify-between hover:border-amber-500/30 transition-colors">
            <div>
              <p className="font-semibold text-white">{driver.name}</p>
              <p className="text-sm text-gray-400">{driver.phone || 'Телефон не указан'}</p>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => startEdit(driver)} className="text-gray-600 hover:text-amber-400 transition-colors">
                <Pencil size={15} />
              </button>
